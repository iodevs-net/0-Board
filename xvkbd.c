 ...
 *
 * Copyright (C) 2000-2015 by Tom Sato <VEF00200@nifty.ne.jp>
...
#include "resources.h"
#define PROGRAM_NAME_WITH_VERSION "xvkbd (v3.7)"
...
static XtResource application_resources[] = {
  { "description", "Description", XtRString, sizeof(char *),
    ...
    "Copyright (C) 2000-2013 by Tom Sato <VEF00200@nifty.ne.jp>\n"
    "http://homepage3.nifty.com/tsato/xvkbd/\n\n"
...
static void ReadKeymap(void)
{
...
#ifdef OLD_MODE_SWITCH_HACK
	mode_switch_mask = 0x0101 << i;
#else
  ...
  if (level3_shift_mask == mode_switch_mask) {
    ...
    if (appres.debug) {
      fprintf(stderr, "%s: both ISO_Level3_Shift and Mode_switch found\n", PROGRAM_NAME);
      fprintf(stderr, "%s: assuming ISO_Level3_Shift=0x%x, Mode_switch=0x%x\n",
	      PROGRAM_NAME, level3_shift_mask, mode_switch_mask);
    }
  }
...
#ifdef OLD_MODE_SWITCH_HACK
    /* I'm not sure, but maybe I need to use ISO_Level3_Shift in some old systems. */
    /* Only in such cases, -DOLD_MODE_SWITCH_HACK may be enabled. */
      if (appres.debug)
...
static void SendFileContent(const char *file)
{
  ...
  while ((ch = fgetc(fp)) != EOF) {
    if (0 < appres.text_delay) usleep(appres.text_delay * 1000);
    if (appres.utf16) {  /* UTF-16 encoded text */
      ch2 = fgetc(fp);
      if (first) {
	first = FALSE;
	if (ch == 0xff && ch2 == 0xfe) little_endian = TRUE;
	else if (ch == 0xfe && ch2 == 0xff) continue;
      }
      if (little_endian)
	ch = (ch2 ? 0x1000000 : 0) | ch2 << 8 | ch;
      else
	ch = (ch ? 0x1000000 : 0) | ch << 8 | ch2;
    }
    if (ch == '\n') {  /* newline - send Return instead */
      SendKeyPressedEvent(XK_Return, 0, 0);
    } else if (ch == '\033') {  /* ESC */
      SendKeyPressedEvent(XK_Escape, 0, 0);
    } else if (ch == '\177') {  /* DEL */
      SendKeyPressedEvent(XK_Delete, 0, 0);
    } else if (1 <= ch && ch <= 26) {  /* Ctrl-x */
      SendKeyPressedEvent('a' + ch - 1, ControlMask, 0);
...
static void LayoutSelected(Widget w, char *key, char *data)
{
  ...
  if (toplevel != None && XtWindow(toplevel) != None) {
    for (i = 1; i < argc1; i++) {
...
static void OpenRemoteDisplay(Widget w, char *display_name, char *data)
{
  ...
  *cp = '\0';
  ...
  if (strlen(name) == 0) {
	...
	fprintf(stderr, "%s: Couldn't connect to the display: %s\n",
		PROGRAM_NAME, name);
      }
      target_dpy = dpy;
    } else {
...
#include "iconify.xbm"
  ...
  if (!appres.resizable) {
    Dimension wd, ht;
    XtVaGetValues(toplevel, XtNwidth, &wd, XtNheight, &ht, NULL);
    XtVaSetValues(toplevel, XtNminWidth, wd, XtNmaxWidth, wd,
		  XtNminHeight, ht, XtNmaxHeight, ht, NULL);
  }
...
static void ReadProperty(void)
{
  ...
  while (fgets(str, sizeof(str) - 1, fp)) {
	...
	keyboard_layout = XtNewString(key);
	if (appres.debug) fprintf(stderr, "ReadProperty: keyboard=%s, XENVIRONMENT=%s\n",
...
static void SaveProperty(void)
{
  ...
  if (appres.debug) fprintf(stderr, "SaveProperty\n");
  ...
  if (keyboard_layout != NULL) fprintf(fp, "#keyboard %s\n", keyboard_layout);
...
static void PopupFunctionKeyEditor(void)
{
  ...
  if (edit_fkey_panel == None) {
    ...
    XtAddCallback(button, XtNcallback, (XtCallbackProc)SaveProperty, NULL);
...
#include "XVkbd-common.h"
  ...
  if (strlen(appres.remote_display) != 0)
    OpenRemoteDisplay(None, appres.remote_display, NULL);

  if (0 < strlen(appres.window)) {
    if (strcmp(appres.window, "root") == 0) {
      focused_window = RootWindow(target_dpy, DefaultScreen(dpy));
    } else if (sscanf(appres.window, "0x%lX%c", &focused_window, &ch) != 1) {
      if (sscanf(appres.window, "%ld%c", &focused_window, &ch) != 1) {
        focused_window = FindWindow(RootWindow(target_dpy, DefaultScreen(target_dpy)),
                                    appres.window);
        if (focused_window == None) {
